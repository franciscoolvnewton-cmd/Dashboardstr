...
pio.templates["veros_light"] = TEMPLATE_LIGHT
pio.templates["veros_dark"] = TEMPLATE_DARK
pio.templates.default = "veros_light"
px.defaults.template = "veros_light"
...
def obter_cores_por_tema(tema):
    ...
    if tema == 'dark':
        return {
            ...
            'palette': ['#60a5fa', '#34d399', '#fbbf24', '#f87171', '#a78bfa', '#22d3ee', '#f97316', '#facc15']
        }
    else:
        return {
            'primary': '#2563eb',
            ...
            'gradient_3': 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)',
            'gradient_4': 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)',
            'palette': ['#2563eb', '#059669', '#f97316', '#dc2626', '#7c3aed', '#14b8a6', '#facc15', '#6366f1']
        }
...
def aplicar_tema_plotly():
    """Atualiza tema padrão do Plotly para combinar com o tema atual do dashboard."""
    template_name = "veros_dark" if TEMA_ATUAL == 'dark' else "veros_light"
...
def criar_heatmap_interativo(matriz, titulo):
    ...
    try:
        ...
        fig.update_xaxes(title_text="Mês de Geração da Receita", type="category", categoryorder="array", categoryarray=x_labels)
        fig.update_yaxes(title_text="Mês de Geração do Lead", type="category", categoryorder="array", categoryarray=y_labels)
...
def criar_heatmap_matriz(matriz, titulo, colorscale='Blues', width=500, height=450):
    ...
    fig.update_xaxes(
        title_text="Mês de Geração da Receita",
        tickangle=45,
    ...
    fig.update_yaxes(
        title_text="Mês de Geração do Lead",
        tickfont=dict(size=10, color=COLORS['text_primary']),
        type="category",
        categoryorder="array",
...
def configurar_layout_clean(fig, titulo="", width=800, height=500, fonte_maior=False, show_labels=True):
    ...
    if fonte_maior:
        ...
        fig.update_layout(
            ...
            width=width,
            height=height,
            colorway=COLORS.get('palette', px.colors.qualitative.D3)
        )
    else:
        # Configuração padrão
        fig.update_layout(
        ...
        for trace in fig.data:
            if isinstance(trace, go.Bar):
                ...
                marker_conf = dict(
                    color=marker_color,
                    opacity=0.9,
                    line=dict(width=1.2, color=COLORS['background'])
                )
                ...
                trace.update(
                    ...
                    marker=dict(
                        size=8,
                        line=dict(width=2, color=COLORS['paper_bg']),
                        symbol="circle",
                        opacity=0.95
                    ),
                    hovertemplate="<b>%{x}</b><br>Valor: %{y:,.0f}<extra></extra>"
